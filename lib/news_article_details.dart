...
class _NewsDetailScreenState extends State<NewsDetailScreen> {
  ...
  void initState() {
    ...
    _newsViewsCount = widget.news.viewsCount;
    _loadInitialData();
    _refreshTimer = Timer.periodic(ApiConfig.refreshIntervalSlow, (_) => _refreshComments());
  }
  ...
  Future<void> _loadInitialData({bool showMainLoader = true}) async {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _newsViewsCount = freshNews.viewsCount;
        });
      }
    } catch (e) {
      // Handle error
    ...
    if (!_viewRecorded) {
      _viewRecorded = true;
      final updatedViews = await NewsService.recordView(widget.news.id);
      if (mounted && updatedViews != null) {
        setState(() => _newsViewsCount = updatedViews);
      }
    }
  ...
  String _formatTimeAgo(String dateString) => formatTimeAgoFromString(dateString);
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              ...
                              const SizedBox(width: 6),
                              Text(
                                '$_newsViewsCount ${_newsViewsCount == 1 ? 'view' : 'views'}',
                                style: const TextStyle(
                                  color: Colors.white54,
                                  fontSize: 14,
                                ),
  ...
  Widget _buildLoginPrompt() {
    return Container(
      margin: EdgeInsets.all(DarLayoutMetrics.of(context).horizontalPadding),
  ...
  Widget _buildComment(Comment comment, {int level = 0}) {
    ...
    return Container(
      margin: EdgeInsets.fromLTRB(
        isNested ? DarLayoutMetrics.of(context).horizontalPadding : DarLayoutMetrics.of(context).horizontalPadding,
        8.0,
