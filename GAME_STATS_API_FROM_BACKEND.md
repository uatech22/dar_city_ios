...
Create a live session after coach picks **match lineup (5–12)** and **starting five + captain**.

**Request body:**
```json
{
  "match_id": 42,
  "match_lineup_player_ids": [12, 7, 3, 15, 22, 9, 11, 14],
...
| `match_lineup_player_ids` | integer[] | yes | Length **5–12**, unique, subset of club roster — writes **lineup table only** |
| `starting_five_player_ids` | integer[] | yes | Length **exactly 5**, subset of `match_lineup_player_ids` — writes **on-court table** |
| `captain_player_id` | integer | yes | Must be one of `starting_five_player_ids` |
...
**Keep match lineup and on-court state in separate tables.** Do not store full club squad in session lineup. Do not update lineup membership on substitution — only flip who is active on court.
...
-- TABLE 1: Match lineup (fixed when session is created, 5–12 players)
game_stats_session_lineup
  session_id, player_id, is_starter, sort_order
  UNIQUE(session_id, player_id)
  -- is_starter = true for the initial starting five only (never changes on sub)

-- TABLE 2: Active on court (exactly 5 rows per session, changes on substitution)
game_stats_session_on_court
  session_id, player_id, sort_order
  UNIQUE(session_id, player_id)
  -- On sub: DELETE player_out row, INSERT player_in row (player_in must exist in lineup table)
...
**Derived (do not store as source of truth):**
- `bench` = players in `game_stats_session_lineup` NOT IN `game_stats_session_on_court`
- `GET /matches/{id}/roster` = full club squad (picker only) — never write this into session lineup

**API snapshot mapping:**
| DB | JSON field |
|----|------------|
| `game_stats_session_lineup` | `match_lineup` / `match_lineup_player_ids` (alias `roster` ok) |
| `game_stats_session_on_court` | `on_court` / `on_court_player_ids` |
| derived | `bench` / `bench_player_ids` |

**Substitution rule:** Only touch `game_stats_session_on_court`. Never add/remove `game_stats_session_lineup` rows during live play.

**Anti-pattern (causes your bug):** Single table with only `is_on_court` but rows = full squad roster, or overwriting lineup when subbing.
